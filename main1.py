...
def main():
    ...
    can.drawString(465, highest + 2, str(birthday.replace("-", "/")))
    ...
    can.setFillColorRGB(199 / 255, 12 / 255, 129 / 255)
    can.drawString(137, 270, f"0 - {age_range}")
    can.drawString(232, 270, f"{age_range + 1} - {age_range + 9}")
    can.drawString(185, 323, f"{age_range + 10} - {age_range + 18}")
    can.drawString(185, 388, f"{age_range + 19} - {age_range + 27}")
    ...
    existing_pdf = PdfFileReader(open("result1.pdf", "rb"))
    ...
    outputStream.close()
    draw_page_info(full_name, birthday)
    draw_page_bia(full_name, birthday)
    draw_page_Chao(full_name)

def draw_page_bia(name, phone):
    packet = io.BytesIO()
    can = canvas.Canvas(packet, pagesize=(612, 900))
    # Chiều cao default text ban đầu
    fullname_front = 'Vietnamese-coiny-Front'
    ...
    # Drawing Name & Date
    can.setFont(fullname_front1, 28)
    can.setFillColor(reportlab.lib.colors.purple)
    width = 300
    width = width - len(name) * 6
    can.drawString(width, 100, str(name).upper())
    can.setFont(fullname_front1, 26)
    can.drawString(250, 60, str(phone.replace("-", "/")))
    can.save()
    ...
    new_pdf = PdfFileReader(packet)
    # read your existing PDF
    existing_pdf = PdfFileReader(open("pdfteamplate/Page_01.pdf", "rb"))
    output = PdfFileWriter()
    page = existing_pdf.getPage(0)
    page.mergePage(new_pdf.getPage(0))
    output.addPage(page)
    outputStream = open("pdfteamplate/Page_01_Edit.pdf", "wb")
    output.write(outputStream)
    outputStream.close()

def draw_page_info(name, phone):
    packet = io.BytesIO()
    can = canvas.Canvas(packet, pagesize=(612, 900))
    # Chiều cao default text ban đầu
    fullname_front = 'Vietnamese-coiny-Front'
    ...
    can.setFont(fullname_front1, 18)
    can.setFillColor(reportlab.lib.colors.purple)
    can.drawString(210, 330, str(name).upper())
    can.drawString(210, 288, str(phone.replace("-", "/")))
    can.save()
    ...
    new_pdf = PdfFileReader(packet)
    # read your existing PDF
    existing_pdf = PdfFileReader(open("pdfteamplate/Page_02.pdf", "rb"))
    output = PdfFileWriter()
    page = existing_pdf.getPage(0)
    page.mergePage(new_pdf.getPage(0))
    output.addPage(page)
    outputStream = open("pdfteamplate/Page_02_Edit.pdf", "wb")
    output.write(outputStream)
    outputStream.close()

def draw_page_Chao(name):
    packet = io.BytesIO()
    can = canvas.Canvas(packet, pagesize=(612, 900))
    # Chiều cao default text ban đầu
    fullname_front = 'Vietnamese-coiny-Front'
    ...
    # Drawing Name & Date
    can.setFont(fullname_front1, 22)
    can.setFillColor(reportlab.lib.colors.black)
    can.drawString(125, 746, str(name).upper())
    can.save()
    ...
    new_pdf = PdfFileReader(packet)
    # read your existing PDF
    existing_pdf = PdfFileReader(open("pdfteamplate/Page_04.pdf", "rb"))
    output = PdfFileWriter()
    page = existing_pdf.getPage(0)
    page.mergePage(new_pdf.getPage(0))
    output.addPage(page)
    outputStream = open("pdfteamplate/Page_04_Edit.pdf", "wb")
    output.write(outputStream)
    outputStream.close()
...
if __name__ == '__main__':
    main()
    _pdfs = []
    _pdfs.append('pdfteamplate/Page_01_Edit.pdf')
    _pdfs.append('pdfteamplate/Page_02_Edit.pdf')
    _pdfs.append('pdfteamplate/Page_03.pdf')
    _pdfs.append('pdfteamplate/Page_04_Edit.pdf')
    _pdfs.append('pdfteamplate/Page_05.pdf')
    _pdfs.append('pdfteamplate/Page_06.pdf')
    _pdfs.append('pdfteamplate/Page_07.pdf')
    _pdfs.append('pdfteamplate/Page_08.pdf')
    _pdfs.append('pdfteamplate/Page_09.pdf')
    _pdfs.append('pdfteamplate/Page_10.pdf')
    _pdfs.append('pdfteamplate/Page_11.pdf')
    _pdfs.append('pdfteamplate/Page_12.pdf')
    _pdfs.append('pdfteamplate/Page_13.pdf')
    _pdfs.append('pdfteamplate/Page_14.pdf')
    _pdfs.append('pdfteamplate/Page_15.pdf')
    _pdfs.append('output.pdf')
    _pdfs.append('pdfteamplate/Page_LoiCamOn.pdf')
    _pdfs.append('pdfteamplate/Page_BiaSau.pdf')
    merge_pdfs(_pdfs)
